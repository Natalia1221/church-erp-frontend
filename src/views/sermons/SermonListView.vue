            ...
            >
              ...
              <td class="py-3.5 px-4 text-center">
                <div
                  ...
                  class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-700 font-semibold text-xs"
    ...
    <Teleport to="body">
      ...
      >
        ...
        <div v-if="duplicatePopup.existingTitle" class="p-3 rounded-xl bg-slate-50 border border-slate-200 text-left space-y-1">
          ...
          <p class="text-xs font-mono font-bold text-indigo-700">{{ duplicatePopup.existingTitle }}</p>
...
const showDuplicatePopup = (date, existingTitle, customMsg = '') => {
  ...
  duplicatePopup.message = customMsg || `Acara Sermon untuk tanggal ${formatTanggal(date)} sudah terdaftar di sistem. Anda tidak dapat membuat acara Sermon ganda pada tanggal yang sama.`
}
...
// Saat tanggal diinput, otomatis bentuk title SERMON_TANGGAL dan cek duplikasi
const onDateChange = () => {
  if (form.event_date) {
    form.title = `SERMON_${form.event_date}`
    const existing = sermons.value.find(s => s.event_date === form.event_date)
    if (existing) {
...
const submitCreateSermon = async () => {
  ...
  // Validasi: Tidak boleh ada acara Sermon dengan tanggal yang sama
  const existing = sermons.value.find(s => s.event_date === form.event_date)
  if (existing) {
    showDuplicatePopup(form.event_date, existing.title)
    return
  ...
  try {
    ...
    const errorMsg = error.response?.data?.message || error.message || 'Gagal membuat acara sermon'
